        ...
        <div v-if="isDataLoaded" class="bg-white min-h-screen rounded-xl p-7 ml-7">
            ...
            <div class="items-center justify-center p-2">
                <div class="flex justify-between items-center pt-4  pb-7">
                    <div @click="openModal" class="flex bg-cyan-800 text-white mb-5 md:mb-0 h-10 rounded-xl items-center px-3 text-xs xl:text-sm">
                        <vue-feather type="plus" size="18" />
                        <p class=" m-2">Create</p>
                    </div>
                
                <!-- Filter Section -->
                <div class="flex gap-3 items-center justify-center pb-5 text-xs xl:text-sm">
                    <div class="relative text-gray-500">
                        <input v-model="searchKeyword" type="search" name="search" placeholder="Search..."
                            class="bg-white h-10 w-full xl:w-64 px-5 rounded-lg border text-sm focus:outline-none" />
                       <button type="submit" @click="searchPengeluaran" class="absolute right-0 top-0 mt-3 mr-4 focus:outline-none hover:text-purple-600">
                            ...
                            <div class="overflow-x-auto sm:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-200 text-left">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200 border-t border-gray-300">
                                        <tr v-if="filteredPengeluaran.length === 0" class="border-b border-gray-200">
                                        ...
                                        <tr v-for="(pengeluaran, index) in filteredPengeluaran" :key="index"
                                            ...
                                            <td class="px-4 py-5 whitespace-no-wrap  ">
                                                <div class=" text-xs xl:text-sm leading-5 font-medium text-gray-900">{{ formatHarga(pengeluaran.total) }}</div>
                                            </td>
                                           
                                            <td class="px-4 py-5 whitespace-no-wrap text-center flex gap-3 items-center justify-center">
                                              <vue-feather type="trash-2" size="20" stroke="red" @click="openDeleteConfirmationModal(pengeluaran.date)" />
                ...
                <div class="flex justify-end mt-4">
                    <button @click="prevPage" :disabled="pageIndex === 0" class="text-xs cursor-pointer bg-gray-300 p-2 w-20 rounded">
                        Previous
                    </button>
                    <div class="mx-2 p-2 text-xs">
                        Page {{ pageIndex + 1 }} of {{ totalPages}}
                    </div>
                    <button @click="nextPage" :disabled="pageIndex === totalPages - 1" class="text-xs cursor-pointer bg-gray-300 p-2 w-20 rounded">
                        Next
                    </button>
                </div>
            </div>
