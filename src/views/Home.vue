  ...
  >
    <div class="bg-white min-h-screen rounded-xl p-7 ml-7">
      <div class="lg:flex justify-between items-center mb-8 p-2 pt-8 pb-5">
        <p class="text-xl xl:text-2xl font-semibold mb-2 lg:mb-0 text-gray-800">
      ...
      <div class="flex flex-wrap -mx-3 mb-16 p-2">
        <div class="w-1/2 xl:w-1/4 px-3">
          ...
          >
            <div
              class="px-3 py-10 lg:px-5 lg:py-8 bg-pink text-white rounded-l-lg"
            >
              <font-awesome-icon
                icon="cube"
        ...
        <div class="w-1/2 xl:w-1/4 px-3">
          ...
          >
            <div
              class="px-3 py-10 lg:px-5 lg:py-8 bg-purple text-white rounded-l-lg"
            >
              <font-awesome-icon
                icon="cart-shopping"
                class="w-8 h-8 fill-current mx-auto hidden lg:block"
        ...
        <div class="w-1/2 xl:w-1/4 px-3">
          ...
          >
            <div
              class="px-3 py-10 lg:px-5 lg:py-8 bg-orange text-white rounded-l-lg"
            >
              <font-awesome-icon
                icon="truck-fast"
                class="w-8 h-8 fill-current mx-auto hidden lg:block"
        ...
        <div class="w-1/2 xl:w-1/4 px-3">
          ...
          >
            <div
              class="px-3 py-10 lg:px-5 lg:py-8 bg-green text-white rounded-l-lg"
            >
              <font-awesome-icon
                icon="sack-dollar"
                class="w-8 h-8 fill-current mx-auto hidden lg:block"
      ...
      <div class="flex flex-wrap -mx-6 p-6">
        ...
        <div class="w-full xl:w-1/2 px-3">
          ...
          <div class="w-full bg-white border rounded-lg p-4">
            <div
              v-for="(transaction, index) in top3Transactions"
              :key="index"
              class="w-full bg-slate-100 border rounded-lg flex justify-between items-center px-4 py-2 mb-4"
            >
              <div>
                <p class="font-medium text-base">{{ transaction.nama_toko }}</p>
                <p class="text-sm text-cyan-800">
                  {{ transaction.totalPenjualan }} kayu
                </p>
              </div>
              <span
                :class="
                  transaction.totalHarga > 0 ? 'text-green-500' : 'text-pink'
                "
                class="font-semibold text-lg"
                >{{ formatHarga(transaction.totalHarga) }}</span
              >
            </div>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const top3Transactions = computed(() => {
      const sortedTransactions = [...recentTransactions.value].sort(
    ...
    const fetchRecentTransactions = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_APP_BASE_URL}/penjualan/top`
        );
