...
export default {
  setup() {
    ...
    const totalPages = computed(() =>
      Math.ceil(penjualan.value.length / itemsPerPage)
    );
    ...
    const printPenjualanData = () => {
      ...
      penjualan.value.forEach((item) => {
        const jenisProduk = item.penjualanItems
          .map((prod) => prod.product.jenis_product)
          .join(", ");
        const namaProduk = item.penjualanItems
          .map((prod) => prod.product.nama_product)
          .join(", ");
        const ukuranProduk = item.penjualanItems
          .map((prod) => prod.product.ukuran_product)
          .join(", ");
        const jumlahProduk = item.penjualanItems
          .map((prod) => prod.quantity.toString().replace(".", ","))
          .join(", ");
        const hargaProduk = item.penjualanItems
          .map((prod) => formatHarga(prod.product.hargaJual))
          .join(", ");
        const diskon =
          item.diskon !== null ? formatHarga(item.diskon) : "Tidak Ada Diskon";
    ...
    const filteredPenjualan = computed(() => {
      return penjualan.value.filter((item) => {
        ...
        const additionalFields = [
          "penjualanItems.product.nama_product",
          "penjualanItems.product.jenis_product",
        ];
              ...
              <table class="min-w-full divide-y divide-gray-200 text-left">
                <thead>
                  <tr class="capitalize">
                    ...
                    <th
                      class="px-6 py-5 bg-cyan-600 text-white text-xs xl:text-sm leading-4 font-medium tracking-wider"
                    >
                      Tanggal
                    </th>
                    <th
                    ...
                    <th
                      class="px-6 py-5 bg-cyan-600 text-white text-center text-xs xl:text-sm leading-4 font-medium tracking-wider"
                    >
                      Aksi
                    </th>
                  </tr>
                ...
                >
                  <tr
                    v-if="displayedPenjualan.length === 0"
                  ...
                  <tr
                    v-else
                    v-for="(item, index) in displayedPenjualan"
                  ...
                  >
                    ...
                    <td class="px-6 py-4 whitespace-no-wrap">
                      <div
                        class="text-xs xl:text-sm leading-5 font-medium text-gray-900"
                        v-if="item.diskon !== null"
                      >
                        {{ formatHarga(item.diskon) }}
                      </div>
                      <div
                        class="text-xs xl:text-sm leading-5 font-medium text-gray-900"
                        v-else
                    ...
                    <td class="px-6 py-4 whitespace-no-wrap">
                      <div
                        class="text-xs xl:text-sm leading-5 font-medium text-gray-900"
                      >
                        {{ formatHarga(item.totalHarga_product) }}
                      </div>
                    </td>
